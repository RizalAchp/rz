...
static bool rz__sub_timespec(RZ_OsSystemTime self, RZ_OsSystemTime other, RZ_Duration *result) {
    RZ_Duration self_dur  = rz__timespec2duration(self);
    RZ_Duration other_dur = rz__timespec2duration(other);
    if (rz_duration_cmp(self_dur, other_dur) >= 0) {
        if (self.tv_nsec >= other.tv_nsec) {
            RZ_DEBUG_ASSERT(self.tv_sec >= other.tv_sec);
            result->secs  = self.tv_sec - other.tv_sec;
            result->nanos = self.tv_nsec - other.tv_nsec;
        } else {
            RZ_DEBUG_ASSERT(self.tv_nsec < other.tv_nsec);
            RZ_DEBUG_ASSERT(self.tv_sec > other.tv_sec);
            RZ_DEBUG_ASSERT(self.tv_sec > RZ_I64_MIN);
            RZ_DEBUG_ASSERT((self.tv_sec - 1) >= other.tv_sec);
            result->secs  = (self.tv_sec - 1) - other.tv_sec;
            result->nanos = self.tv_nsec + ((rz_u32)RZ_TIME_NSEC_PER_SEC) - other.tv_nsec;
        }
        return true;
    } else {
        return !rz__sub_timespec(other, self, result);
    }
}
#endif
...
RZ_DEF RZ_InstantTime rz_instant_now(void) {
    ...
    if (!rz__timespec(t.tv_sec, t.tv_nsec, &result.t)) RZ_PANIC("invalid timestamp");
#else
...
#elif defined(RZ_OS_UNIX)
    return rz__sub_timespec(self.t, earlier.t, result);
#else
#    error "unimplemented for this os"
#endif
}
...
#elif defined(RZ_OS_UNIX)
    RZ_Duration res = {0};
    if (!rz_duration_checked_add(rz__timespec2duration(self.t), duration, &res)) return false;
    result->t = rz__duration2timespec(res);
    return true;
#else
#    error "unimplemented for this os"
#endif
...
#elif defined(RZ_OS_UNIX)
    RZ_Duration res = {0};
    if (!rz_duration_checked_sub(rz__timespec2duration(self.t), duration, &res)) return false;
    result->t = rz__duration2timespec(res);
    return true;
#else
#    error "unimplemented for this os"
...
RZ_DEF bool rz_time_is_leap_year(rz_i32 year) {
...
RZ_DEF rz_u16 rz_time_days_in_year(rz_i32 year) {
    return (rz_time_is_leap_year(year)) ? 366 : 365;
}

RZ_DEF rz_u8 rz_time_weeks_in_year(rz_i32 year) {
    // clang-format off
...
RZ_DEF rz_u8 rz_time_days_in_month(rz_u8 month, rz_i32 year) {
    return rz_time_days_in_month_leap(month, rz_time_is_leap_year(year));
}

RZ_DEF rz_u8 rz_time_days_in_month_leap(rz_u8 month, bool is_leap_year) {
